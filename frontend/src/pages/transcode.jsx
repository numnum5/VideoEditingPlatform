...
function VideoList({setSelectedVideo, handleReset, data, setData}){
  ...
  useEffect(()=>{
    fetch(`${apiUrl}/api/playlists/${sessionStorage.getItem('username')}`)
    .then(res => {
      if (!res.ok) {
  ...
  const fetchVideos = async(page, playlist_id, sortBy) => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/videos/uservideos?page=${page}&playlist=${playlist_id}&limit=${limit}&sortBy=${sortBy}`, {
        method: 'GET',
        headers: {
...
export default function Edit({loggedState}) {
  ...
  //       setError(new Error('Error connecting to the server. Please try again later.'));
  //       setInProgres(false);
  //     };
  ...
  //         const newVideo = {
  //           presignedUrl : update.presignedUrl,
  //           outputKey : update.key
  //         }
  ...
  const handleDelete = (e) => {
    ...
    if(!inProgress && selectedVideo){
      // Delete video
        fetch(`${apiUrl}/api/videos/delete/${selectedVideo.id}`, {
          method: "DELETE", 
          headers: {
              "Content-Type": "application/json",
              'Authorization': `Bearer ${sessionStorage.getItem('idToken')}`
  ...
  // Function submitting informatio to the server
    ...
    const handleSubmit = (e) => {
      ...
      if (!inProgress && selectedVideo) {
        ...
        const newEventSource = new EventSource(`${apiUrl}/progress/progress/${selectedVideo.filename}`);
        ...
        newEventSource.onmessage = (event) => {
          ...
          if (parsed.status === "Pending") {
              ...
              setInProgres(false);
              newEventSource.close(); // Close connection when finished
          }
        };
        newEventSource.onerror = () => {
          setError(new Error('Connection to the server lost. Please try again.'));
          setInProgres(false);
          newEventSource.close();
      };
      
      // Submit the processing request to the server
      fetch(`http://localhost:3000/videos/process`, {
        method: 'POST',
        headers: {
            Authorization: `Bearer ${sessionStorage.getItem('idToken')}`,
            'Content-Type': 'application/json'
        ...
        body: JSON.stringify({
            ...
            socketId: selectedVideo.filename, // Using the video ID as a unique identifier
...
function Results({editedVideo, selectedVideo, handleReset, inProgress, setCompleted}){
  ...
  function handleSave(e){
    ...
    // Update video
    fetch(`${apiUrl}/api/videos/update/${selectedVideo.id}`, {
      method: 'PUT',
      headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${sessionStorage.getItem('idToken')}`
  ...
  function handleDownload(e){
    ...
    if(!inProgress && !error){
      ...
      fetch(`${apiUrl}/api/videos/download/${editedVideo.filename}`) // Convert the response to a Blob (binary large object)
      .then(res => res.blob())
      .then(blob => {
  ...
  return(      
    <div>
      <div style={{ marginBottom : '10px'}}>
        <video controls src={editedVideo.presignedUrl}>
        </video>
      </div>
